...
function EFFECT:Init(data)
    ...
    if game.SinglePlayer() then flProjectileDelay = cl_portal_projectile_delay_sp:GetFloat() end
    ...
    if not data.PlacedByPedestal then
        if owner:IsPlayer() then
            ...
            if IsValid(self.Particle) then self.Particle:SetControlPoint(2, data:GetNormal()) end
        end
    end
end
