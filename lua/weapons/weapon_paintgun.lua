...
function SWEP:Deploy()
    timer.Simple(0.1, function() self:GetOwner():GetViewModel(0):SetColor4Part(255, 5, 255, 255) end)
    return true
end
...
function SWEP:PrimaryAttack()
    ...
    if SERVER then self:DoTraceToSurface() end
end
...
function SWEP:Think()
    if SERVER then if CurTime() > self.NextIdleTime and self:GetActivity() ~= ACT_VM_IDLE then self:SendWeaponAnim(ACT_VM_IDLE) end end
end
