...
if SERVER then CreateConVar("gp2_portal_placement_never_fail", "0", FCVAR_CHEAT + FCVAR_NOTIFY, "Can portal be placed on every surface?") end
concommand.Add("gp2_change_linkage_group_id", function(ply, cmd, args)
    if SERVER then
        local wep = ply:GetActiveWeapon()
        if IsValid(wep) then if wep:GetClass() == "weapon_portalgun" then wep:SetLinkageGroup(tonumber(args[1]) or 0) end end
    end
end)
...
local function setPortalPlacementNew(owner, portal)
    ...
    if not gp2_portal_placement_never_fail:GetBool() and (not tr.Hit or IsValid(tr.Entity) or tr.HitTexture == "**studio**" or bit.band(tr.SurfaceFlags, SURF_NOPORTAL) ~= 0 or bit.band(tr.SurfaceFlags, SURF_TRANS) ~= 0) then --or bit.band(tr.DispFlags, DISPSURF_WALKABLE) ~= 0
    ...
    local angTab = {af * siz[1], -af * siz[1], au * siz[2], -au * siz[2]}
    for i = 1, 4 do
        ...
        if extr.Hit then tr.HitPos = tr.HitPos + angTab[i] * (1 - extr.Fraction) end
    end
...
function SWEP:Initialize()
    ...
    if SERVER then self.NextIdleTime = 0 end
end
...
function SWEP:Deploy()
    ...
    if self.GotEntityInUse then
        ...
        timer.Simple(0, function() vm0:SendViewModelMatchingSequence(12) end)
    end
    return true
end
...
function SWEP:Holster(arguments)
    if SERVER then
        ...
        timer.Simple(0, function()
            if not IsValid(self) then -- Stop erroring on death!
                return
            end
...
function SWEP:PrimaryAttack()
    ...
    if IsValid(self:GetOwner()) and self:GetOwner():IsPlayer() then self:GetOwner():ViewPunch(Angle(math.Rand(-1, -0.5), math.Rand(-1, 1), 0)) end
...
function SWEP:PlacePortal(type, owner)
    ...
    portal:SetPlacedByMap(false)
    if PORTAL_USE_NEW_ENVIRONMENT_SYSTEM then portal:BuildPortalEnvironment() end
    --- @type Player
    local player = owner
...
function SWEP:ClearPortals()
    ...
    if SERVER then
        if IsValid(portal1) and self:GetCanFirePortal1() then portal1:Fizzle() end
        if IsValid(portal2) and self:GetCanFirePortal2() then portal2:Fizzle() end
    end
...
function SWEP:ViewModelDrawn(vm)
    ...
    local vm0 = owner:GetViewModel(0)
    local vm1 = owner:GetViewModel(1)
    if not self.TopLightFirstPersonAttachment then self.TopLightFirstPersonAttachment = vm0:LookupAttachment("Body_light") end
    if not self.TopLightFirstPerson2Attachment then self.TopLightFirstPerson2Attachment = vm1:LookupAttachment("Body_light") end
    ...
    if not IsValid(self.TopLightFirstPerson) then
        ...
        if vm == vm0 then self.TopLightFirstPerson:Render() end
    end
    ...
    if not IsValid(self.TopLightFirstPerson2) then
        ...
        if vm == vm1 then self.TopLightFirstPerson2:Render() end
    end

    if vm0:GetModel() ~= vm1:GetModel() then -- Fix stupid holding particle bug
        return
    end
...
function SWEP:Reload()
    ...
    local portal1 = PortalManager.GetLinkageGroup(self:GetLinkageGroup())[PORTAL_TYPE_FIRST]
    local portal2 = PortalManager.GetLinkageGroup(self:GetLinkageGroup())[PORTAL_TYPE_SECOND]
    if not (IsValid(portal1) or IsValid(portal2)) then return end
    self:ClearPortals()
    self:SendWeaponAnim(ACT_VM_FIZZLE)
    self.NextIdleTime = CurTime() + 0.5
end
...
local function CalcViewmodelBob(self)
    local cycle = 0
    local plr = self:GetOwner():IsPlayer() and self:GetOwner()
