...
function ENT:HandleAnimEvent(event, eventTime, cycle, type, options)
    local phys = self:GetPhysicsObject()
    if event ~= 1100 or self:GetHeld() then return true end
    if IsValid(phys) then
...
function ENT:Use(activator, caller, useType, value)
    if activator:IsPlayer() and not self:IsPlayerHolding() and not self:GetHeld() then
        if not self:GetIsShortcircuit() then self:ResetSequence("hermit_in") end
    ...
    if SERVER then
        if not self:IsPlayerHolding() and self:GetHeld() then
            ...
            if not self:GetIsShortcircuit() then
                ...
                if not self:GetStartAsBox() then self:ResetSequence("hermit_out") end
            end
        end
...
function ENT:AnimateThink()
    if self:GetIsShortcircuit() then
        if self:IsSequenceFinished() then self:ResetSequence("shortcircuit") end
        return
    end
