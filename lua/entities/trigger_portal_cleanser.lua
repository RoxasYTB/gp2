...
function ENT:Initialize()
    ...
    if self:GetVisible() then self:RemoveEffects(EF_NODRAW) end
end
...
function ENT:StartTouch(ent)
    ...
    if ent:IsPlayer() then
        ...
        for i = 1, #weapons do
            ...
            if IsValid(weapon) and weapon:GetClass() == "weapon_portalgun" then weapon:ClearPortals() end
        end
    end
...
function ENT:Think()
    ...
    for _, ent in ipairs(entsInSphere) do
        ...
        if ent == self or not IsValid(ent) or (ent:IsPlayer() and not ent:Alive()) then continue end
        vortexEnts = vortexEnts + 1
        if vortexEnts > 2 then -- Limit to two vortices
            break
        end
