...
function ENT:Think()
    local time = os.clock()
    self:FireLaser()
    if portal_laser_perf_debug:GetBool() then GP2.Print("EnvPortalLaser :: Think - execution time: %.6f seconds", os.clock() - time) end
...
function ENT:FireLaser()
    ...
    local tr = self:RecursionLaserThroughPortals({
        ...
        filter = {self, "projected_wall_entity", "player", "point_laser_target", "prop_laser_catcher", "prop_laser_relay", self:GetParent()},
...
local function PushPlayerAwayFromLine(player, startPos, endPos, baseForce)
    ...
    if not player:Crouching() then baseForce = baseForce * (playerVelocity / 100) end
    ...
    if player:Crouching() then clampedForce = clampedForce * 2 end
...
function ENT:DamageEntsAlongTheRay(startPos, endPos)
    ...
    for i = 1, #rayInfo do
        ...
        if not (target:IsPlayer() or target:IsNPC() or target:IsNextBot() or DAMAGABLE_ENTS[target:GetClass()]) then continue end
        -- Don't spark
        if DAMAGABLE_ENTS[target:GetClass()] then self:SetShouldSpark(false) end
        ...
        -- Ensure the player is valid and capable of being moved
        if target:IsPlayer() and target:GetMoveType() == MOVETYPE_NOCLIP then continue end
        -- Check if the player is on the ground
...
function ENT:OnStateChange(name, old, new)
    ...
    if IsValid(child) then child:SetState(new) end
end
