...
function ENT:Initialize()
    ...
    if IsValid(parent) and parent:GetClass() == "prop_laser_catcher" or parent:GetClass() == "prop_laser_relay" then self:SetLaserCatcher(parent) end
...
function ENT:KeyValue(k, v)
    if k == "terminalpoint" then self:SetTerminalPoint(tobool(v)) end
...
function ENT:OnTakeDamage(info)
    ...
    if not ALLOWED_CLASSES_TO_DAMAGE[attacker:GetClass()] then return end
    self:PowerOn()
    self:NextThink(CurTime() + 0.1)
end
...
if SERVER then
    function ENT:PowerOn()
        ...
        if IsValid(self:GetLaserCatcher()) then self:GetLaserCatcher():PowerOn() end
    ...
    function ENT:PowerOff()
        ...
        if IsValid(self:GetLaserCatcher()) then self:GetLaserCatcher():PowerOff() end
