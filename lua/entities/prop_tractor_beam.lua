...
function ENT:SetupDataTables()
    ...
    if CLIENT then self:NetworkVarNotify("Enabled", self.OnEnabled) end
end
...
function ENT:Initialize()
    if SERVER then
        ...
        timer.Simple(0, function() if self.StartEnabled then self:Enable() end end)
    end
end
...
if SERVER then
    function ENT:Enable()
        ...
        if self.sndBeam then self.sndBeam:Stop() end
    ...
    function ENT:Disable()
        ...
        self.ArmatureStartTime = CurTime()
        if self.sndBeam then self.sndBeam:Stop() end
        if self.sndSpinLp then self.sndSpinLp:FadeOut(self.RotationDuration) end
        self:SetEnabled(false)
    end

    function ENT:SetLinearForce(force)
        if self.StartLinearForce == force then return end
        force = force or 250
        if self.Beam and IsValid(self.Beam) then self.Beam:SetLinearForce(force) end
...
function ENT:Think()
    ...
    if SERVER then
        ...
        if PropTractorBeam and not PropTractorBeam.IsAdded(self) then PropTractorBeam.AddToRenderList(self) end
    end
...
function ENT:CalculateRotationPose()
    ...
    if curTime > (self.RotationStartTime + self.RotationDuration) then return self.RotationTarget end
    ...
    if linearForceFactor ~= 0.0 then
        ...
        if not isInBounds then return linearForceFactor end
    end
    return rotationGoal
end
...
function ENT:CalculateArmaturePose()
    local curTime = CurTime()
    if curTime > (self.ArmatureStartTime + self.ArmatureDuration) then return self.ArmatureTarget end
    ...
    if self.ArmatureStartTime == armatureEndTime then
        ...
        armatureGoal = (((elapsedTime * elapsedTime * 3.0) - ((elapsedTime * elapsedTime * 2.0) * elapsedTime)) * (self.ArmatureTarget - armatureGoal)) + armatureGoal
    end
...
function ENT:OnRemove()
    timer.Simple(0, function() if not IsValid(self) then if IsValid(self.sndBeam) then self.sndBeam:Stop() end end end)
end
