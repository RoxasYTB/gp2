...
function ENT:Initialize()
    ...
    if not self:HasSpawnFlags(SF_PHYSPROP_MOTIONDISABLED) and IsValid(phys) then phys:Wake() end
end

function ENT:KeyValue(k, v)
    if k == "ExplodeOnTouch" then self:SetExplosionOnTouch(tobool(v)) end
end
...
function ENT:AcceptInput(inputName, activator, caller, data)
    ...
    if inputName == "explode" then self:ExplodeFutbol() end
end
...
function ENT:TimerThink()
    if self:GetTimerActive() then
        if self:GetExplosionTimer() <= exploding_futbol_flash_start_time:GetFloat() then
            ...
            if (CurTime() - self:GetLastFlashTime()) > exploding_futbol_flash_duration:GetFloat() then self:SetLastFlashTime(CurTime()) end
        end
...
function ENT:OnTakeDamage(dmg)
    if not self:GetExplosionOnTouch() then return 0 end
    if self:GetHolder() == EXPLODING_FUTBOL_HELD_BY_SPAWNER or self:GetHolder() == EXPLODING_FUTBOL_HELD_BY_CATCHER then return 0 end
end

function ENT:OnFizzled()
    if exploding_futbol_explode_on_fizzle:GetBool() then self:ExplodeFutbol() end
end

function ENT:PhysicsCollide()
    if self:GetExplosionOnTouch() and self:GetHolder() == EXPLODING_FUTBOL_HELD_BY_NONE then self:SetThink(self.KillThink) end
    print('collide')
end
