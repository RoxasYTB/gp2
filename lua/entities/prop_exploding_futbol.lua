...
function ENT:Think()
	if self.NextThinkFunction and isfunction(self.NextThinkFunction) and CurTime() > self.NextThinkTime then
...
function ENT:ActivateFutbolTimer(timer)
	...
	if timer > 0 then
		...
		self:SetLastTimerSoundTime(CurTime() - 1);
		self:SetLastFlashTime(CurTime());
