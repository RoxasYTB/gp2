...
ENT.__input2func = {
    ["press"] = function(self, activator, caller, data) self:Press() end,
    ["release"] = function(self, activator, caller, data) self:Release() end,
    ["lock"] = function(self, activator, caller, data) self:Lock() end,
    ["unlock"] = function(self, activator, caller, data) self:Unlock() end,
    ["cancelpress"] = function(self, activator, caller, data) self:CancelPress() end,
}
...
function ENT:KeyValue(k, v)
    ...
    if k:StartsWith("On") then
        ...
        if string.find(v, "prop_dynamic", 1, true) then self.NoRetrigger = true end
    end
end
...
function ENT:Press()
    ...
    if SERVER and GP2 and GP2.ButtonLogging then GP2.ButtonLogging.LogActivation("BOUTON PILIER", self:GetName(), self:GetPos(), true) end
...
function ENT:Release()
    ...
    if SERVER and GP2 and GP2.ButtonLogging then GP2.ButtonLogging.LogActivation("BOUTON PILIER", self:GetName(), self:GetPos(), false) end
...
function ENT:Think()
    if SERVER then
        ...
        if self:GetIsPressed() and not playerNearby and self.PlayerNearby then
            ...
            if not self.DelayBeforeReset or self.DelayBeforeReset <= 0 then self:Release() end
        end
