...
ENT.__input2func = {
    ["enable"] = function(self, activator, caller, data) self:Enable() end,
    ["enablegagging"] = function(self, activator, caller, data) self:SetIsGagging(true) end,
    ["enablepickup"] = function(self, activator, caller, data) self:SetEnablePickup(true) end,
    ["disable"] = function(self, activator, caller, data) self:Disable() end,
    ["disablegagging"] = function(self, activator, caller, data) self:SetGagging(false) end,
    ["diasblepickup"] = function(self, activator, caller, data) self:SetEnablePickup(false) end,
    ...
    ["selfdestructimmediately"] = function(self, activator, caller, data) self:SetThink(self.ThinkBreak) end,
    ["firebullet"] = function(self, activator, caller, data) self:Shoot() end,
    ["setmodel"] = function(self, activator, caller, data) self:SetModel(INDEX_TO_MODEL[tonumber(data)] or TURRET_MODEL) end,
}
...
function ENT:Initialize()
    ...
    timer.Simple(0, function() self:SetMaxLookDistance(self:GetRange()) end)
    ...
    self.BarrelAttachments = {self:LookupAttachment("LFT_Gun1_Muzzle"), self:LookupAttachment("RT_Gun1_Muzzle"), self:LookupAttachment("LFT_Gun2_Muzzle"), self:LookupAttachment("RT_Gun2_Muzzle")}
    if self:HasSpawnFlags(SF_START_INACTIVE) then self:SetIsActive(false) end
    ...
    timer.Simple(0, function() self:SetNPCState(NPC_STATE_IDLE) end)
...
function ENT:MovingSuddenly()
    ...
    if phys and IsValid(phys) then
        ...
        if not (self:GetIsGagged() or self:GetIsAsActor()) and (currentVelocitySqr - self.LastVelocity) > 500 ^ 2 then self:EmitSound(TALK_NAMES[TALK_FLUNG]) end
...
function ENT:Think()
    if not self:HasCondition(COND.SEE_ENEMY) or self:HasCondition(COND.ENEMY_TOO_FAR) or self.DistToEnemy > self:GetRange() then self:ClearEnemyMemory() end
    self:PreThink()
    if self.NextThinkFunction and isfunction(self.NextThinkFunction) and CurTime() > self.NextThinkTime then self:NextThinkFunction() end
    self:UpdateFacing()
...
function ENT:SetThink(nextThinkFunc)
    if nextThinkFunc then self.NextThinkFunction = nextThinkFunc end
end
...
function ENT:ThinkAutoSearch()
    ...
    if self:GetIsActive() then
        ...
        if IsValid(self:GetEnemy()) and not self:GetIsAsActor() then self:SetThink(self.ThinkDeploy) end
    else
...
function ENT:ThinkActive()
    ...
    if CurTime() > self.ShotTime then
        ...
        -- Fire the gun
        if canShoot then --[[self:IsEnemyBehindGlass() or--]]
    ...
    if CurTime() < self.ThrashTime and not self:IsFlagSet(FL_DISSOLVING) then
        if self.ShotTime < CurTime() then
            ...
            elseif sv_portal_turret_shoot_at_death:GetBool() then
                ...
                self:SetIdealActivity(self.ActivityOpenIdle)
                if attach then self:Shoot(self:EyePos()) end
            end
...
function ENT:ThinkInactive()
    ...
    if self:IsOnFire() then self:ReturnToLife() end
end
...
function ENT:Shoot(vecSrc, strict)
    ...
    if attach then bullet.Src = attach.Pos end
...
function ENT:GetAttackDamageScale()
    if IsValid(self:GetEnemy()) then
        if self:GetEnemy():IsPlayer() then
            ...
            if self:IsOnSide() then return 1.0 end
            return TURRET_FLOOR_DAMAGE_MULTIPLIER
        end

        if self:GetUseSuperDamageScale() then return TURRET_FLOOR_DAMAGE_MULTIPLIER * 4.0 end
        return TURRET_FLOOR_DAMAGE_MULTIPLIER
    end
    return 1
end
...
function ENT:Use(activator, caller, useType, value)
    if activator:IsPlayer() and not self:IsPlayerHolding() then activator:PickupObject(self) end
end
...
function ENT:OnTakeDamage(info)
    if not self:GetIsActive() or self:GetIsAsActor() then return end
    if self:GetState() < STATE_SEARCH then self:SetThink(self.ThinkDeploy) end
    local phys = self:GetPhysicsObject()
    self.LastSight = CurTime() + FLOOR_TURRET_MAX_WAIT
    if IsValid(phys) and IsValid(info:GetAttacker()) and info:GetAttacker():GetClass() ~= self:GetClass() and (info:GetAttacker():IsPlayer() or info:GetAttacker():IsNPC() or info:GetAttacker():IsNextBot()) then phys:SetVelocity(info:GetDamageForce() / phys:GetMass()) end
    return 0
end

function ENT:TrySpeak(line, skipglobal)
    if not self:GetIsActive() or self:GetIsGagged() or self:GetIsAsActor() then return end
    if self.LastSpeakLine == line and SPEAK_LINES_WITH_NO_REPEAT[line] then return end
    if not SPEAK_LINES_WITH_NO_DELAY[line] then if CurTime() < NEXT_SPEAK_ALLOW_TIME or CurTime() < self.NextSpeakTime then return end end
...
function ENT:OnPhysgunPickup(ply, ent)
    if self:GetPickupEnabled() and self:GetPickupEnabled() then self:TriggerOutput("OnPhysGunPickup") end
    return self:GetPickupEnabled()
end

function ENT:OnPhysgunDrop(ply, ent)
    if self:GetPickupEnabled() and self:GetPickupEnabled() then self:TriggerOutput("OnPhysGunDrop") end
end
