...
if SERVER and AddCSLuaFile then AddCSLuaFile() end
...
if CLIENT then
    ...
    if not PortalRendering then
        ...
        if not success then GP2.Print("Failed to load portalrendering.lua: %s", err or "unknown error") end
    end

    PropPortal = PropPortal or {}
    PropPortal.Portals = PropPortal.Portals or {}
    -- Initialize PropPortal functions if not already present
    if not PropPortal.AddToRenderList then PropPortal.AddToRenderList = function(portal) PropPortal.Portals[portal] = true end end
...
function ENT:SetSize(n)
    ...
    if SERVER and self:GetClass() == "prop_portal" then self:UpdatePhysmesh() end
end
...
if SERVER then
    ...
    function ENT:FindObjectsForTransmission()
        ...
        for _, ent in ipairs(entitiesInRay) do
            if self:ShouldTransmitEntity(ent) then self:ProcessEntityForTransmission(ent) end
        end
    end
    ...
    function ENT:ShouldTransmitEntity(ent)
        ...
        if not GP2.ObjectTransmission or not GP2.ObjectTransmission.IsEnabled() then return false end
        ...
        local portalSize = self:GetSize()
        if entSize.x > portalSize.x * self.ObjectTransmissionSettings.SizeMultiplier or entSize.y > portalSize.y * self.ObjectTransmissionSettings.SizeMultiplier then return false end
        return true
    end
    ...
    function ENT:ProcessEntityForTransmission(ent)
        ...
        local transformedPos, transformedAng = PortalManager.TransformPortal(self, linkedPartner, entPos, entAng)
        local _, transformedVel = PortalManager.TransformPortal(self, linkedPartner, nil, entVel:Angle())
    ...
    function ENT:TransformEntityPosition(ent, newPos, newAng, newVel)
        ...
        if constrained then
            for _, constrainedEnt in pairs(constrained) do
                if constrainedEnt ~= ent and IsValid(constrainedEnt) then
                    local constrainedPos, constrainedAng = PortalManager.TransformPortal(self, self:GetLinkedPartner(), constrainedEnt:GetPos(), constrainedEnt:GetAngles())
        ...
        if IsValid(self:GetLinkedPartner()) then self:GetLinkedPartner():TriggerOutput("OnEntityTeleportToMe", ent) end
    end
    ...
    function ENT:ProcessTransmissionQueue()
        ...
        for i = #self.TransmissionQueue, 1, -1 do
            ...
            self:TransformEntityPosition(transmissionData.entity, transmissionData.transformedPos, transmissionData.transformedAng, transmissionData.transformedVel)
            table.remove(self.TransmissionQueue, i)
    ...
    local function onConfigChanged()
        for _, portal in ipairs(ents.FindByClass("prop_portal")) do
            if IsValid(portal) and portal.UpdateTransmissionSettings then portal:UpdateTransmissionSettings() end
        end
    end
...
function ENT:Initialize()
    if SERVER then
        ...
        if not PORTAL_USE_NEW_ENVIRONMENT_SYSTEM then self:SetPos(self:GetPos() + self:GetAngles():Up() * 7.1) end
        PortalManager.PortalIndex = PortalManager.PortalIndex + 1
        -- Delay physics mesh creation to ensure entity is fully initialized
        timer.Simple(0.1, function() if self:IsValid() then self:UpdatePhysmesh() end end)
    end

    if PORTAL_USE_NEW_ENVIRONMENT_SYSTEM then if SERVER and self:GetPlacedByMap() then self:BuildPortalEnvironment() end end
    ...
    if self.UpdateTransmissionSettings then self:UpdateTransmissionSettings() end
end
...
function ENT:OnRemove()
    ...
    if SERVER and self.PORTAL_REMOVE_EXIT then SafeRemoveEntity(self:GetLinkedPartner()) end
    if CLIENT and IsValid(self.RingParticle) then self.RingParticle:StopEmissionAndDestroyImmediately() end
...
if CLIENT then
    ...
    local function getRenderMesh()
        ...
        if not PortalRendering or not PortalRendering.PortalMeshes then return Mesh(), Mesh() end
        if not PortalRendering.PortalMeshes[4] then
            PortalRendering.PortalMeshes[4] = {Mesh(), Mesh()}
            local invMeshTable = {}
            local meshTable = {}
            local corners = {Vector(-1, -1, -1), Vector(1, -1, -1), Vector(1, 1, -1), Vector(-1, 1, -1)}
            local uv = {Vector(0, 1), Vector(1, 1), Vector(1, 0), Vector(0, 0)}
            for i = 1, 4 do
                table.insert(meshTable, {
                    pos = corners[i % 4 + 1],
                    u = uv[i % 4 + 1].y,
                    v = 1 - uv[i % 4 + 1].x
                })
    ...
    function ENT:Draw()
        ...
        if not self.RENDER_MATRIX then self.RENDER_MATRIX = Matrix() end
        ...
        -- after everything
        --
        if PortalRendering and not PortalRendering.Rendering and IsValid(self.RingParticle) then self.RingParticle:Render() end
    end
    ...
    function ENT:Think()
        ...
        if PropPortal and PropPortal.AddToRenderList and not PropPortal.IsAddedToRenderList(self) then PropPortal.AddToRenderList(self) end
    ...
    hook.Add("NetworkEntityCreated", "seamless_portal_init", function(ent)
        if ent:GetClass() == "prop_portal" then
            ...
            timer.Simple(0, function() incrementPortal(ent) end)
        end
    end)
end
...
function ENT:UpdatePhysmesh()
    ...
    if not PORTAL_USE_NEW_ENVIRONMENT_SYSTEM then
        ...
        if not physicsInitialized then
            local simpleMesh = {Vector(-size.x, -size.y, -size.z), Vector(-size.x, -size.y, size.z), Vector(-size.x, size.y, -size.z), Vector(-size.x, size.y, size.z), Vector(size.x, -size.y, -size.z), Vector(size.x, -size.y, size.z), Vector(size.x, size.y, -size.z), Vector(size.x, size.y, size.z)}
        ...
        if self:GetPhysicsObject():IsValid() then
            ...
            local mesh = {Vector(x0, y0, z0), Vector(x0, y0, z1), Vector(x0, y1, z0), Vector(x0, y1, z1), Vector(x1, y0, z0), Vector(x1, y0, z1), Vector(x1, y1, z0), Vector(x1, y1, z1)}
...
function ENT:OnActivated(name, old, new)
    if SERVER then
        ...
        if new then self:EmitSound(self:GetType() == PORTAL_TYPE_SECOND and "Portal.open_red" or "Portal.open_blue") end
    end
    ...
    if SERVER then
        for _, ent in ipairs(ents.FindByClass("projected_wall_entity")) do
            if IsValid(ent) then ent:SetUpdated(false) end
        end
    end
end

function ENT:SetLinkedPartner(partner)
    if partner == self then return end
    if partner:GetClass() ~= self:GetClass() then return end
    if not partner:GetActivated() then return end
    ...
    if SERVER then
        ...
        for _, ent in ipairs(murs) do
            ...
            if tr.Entity == self and IsValid(partner) then
                touche = true
            elseif dist < 4 and angleDiff < 10 then
                -- tolérance de 4 unités et 10° d'angle
                touche = true
            end
