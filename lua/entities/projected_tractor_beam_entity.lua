...
function ENT:SetupDataTables()
    ...
    if SERVER then self:SetRadius(PROJECTED_BEAM_RADIUS) end
end
...
function ENT:Think()
    ...
    if CLIENT then
        ...
        if ProjectedTractorBeamEntity and ProjectedTractorBeamEntity.IsAdded and ProjectedTractorBeamEntity.AddToRenderList then if not ProjectedTractorBeamEntity.IsAdded(self) then ProjectedTractorBeamEntity.AddToRenderList(self) end end
    end
    ...
    for _, ent in ipairs(hits) do
        if ent:GetClass() == "prop_portal" and IsValid(ent:GetLinkedPartner()) then
            ...
            local hitPos = util.IntersectRayWithOBB(rayStart, (rayEnd - rayStart):GetNormalized(), entry:GetPos(), entry:GetAngles(), mins, maxs) or entry:GetPos()
            ...
            newPos = newPos + eRight * localHit.y + eUp * localHit.z - eForward * 1
            -- droite/gauche
            -- haut/bas
            -- coller au mur
    ...
    if SERVER then
        if foundPortal and bestPortalClonePos and bestPortalCloneAng and bestPortalCloneLinked then
            if not self.PortalClone or not IsValid(self.PortalClone) or self.PortalCloneLinked ~= bestPortalCloneLinked then
                if self.PortalClone and IsValid(self.PortalClone) then self.PortalClone:Remove() end
    ...
    if self.TraceFraction ~= tr.Fraction then
        ...
        if developer:GetBool() then debugoverlay.Cross(tr.HitPos, 16, 0.1, nil, true) end
    end
...
function ENT:OnRemove()
    if CLIENT then
        ...
        if ProjectedTractorBeamEntity and ProjectedTractorBeamEntity.Beams then ProjectedTractorBeamEntity.Beams[self] = nil end
    end

    if SERVER then
        if self.Trigger and IsValid(self.Trigger) then self.Trigger:Remove() end
        if self.PortalClone and IsValid(self.PortalClone) then
            self.PortalClone:Remove()
            self.PortalClone = nil
            self.PortalCloneLinked = nil
...
function ENT:CreateBeam(distance)
    ...
    if SERVER and not self.IsPortalClone then
        ...
        if length < 1 then -- fallback si distance n'est pas encore calculée
            length = MAX_RAY_LENGTH
        end
    ...
    if CLIENT then
        ...
        if ProjectedTractorBeamEntity and ProjectedTractorBeamEntity.AddToRenderList then ProjectedTractorBeamEntity.AddToRenderList(self, self.Mesh) end
    end
    ...
    if SERVER then
        if self.Trigger and IsValid(self.Trigger) then
            ...
            if developer:GetBool() then debugoverlay.BoxAngles(pos, self:WorldToLocal(mins), self:WorldToLocal(maxs), ang, 0.1, self:Get_LinearForce() < 0 and debugReversedColor or debugNormalColor) end
        else
