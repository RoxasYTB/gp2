    ...
    if CLIENT then
        timer.Simple(0, function()
            if not IsValid(self) then
                if IsValid(self.TextureFrameToggle) then self.TextureFrameToggle:Remove() end
...
if SERVER then
    ...
    function ENT:Check(state)
        ...
        if IsValid(self.TextureFrameToggle) then self.TextureFrameToggle:Input("SetTextureIndex", nil, nil, state and "1" or "0") end
    end
end
