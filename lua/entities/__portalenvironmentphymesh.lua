...
function ENT:PrecacheFaces()
    ...
    for i = 1, #faces do
        local face = faces[i]
        if not face:IsWorld() or face:HasTexInfoFlag(0x00001000) then continue end
...
function ENT:RebuildPhymesh()
    ...
    for i = 1, #PRECACHED_FACES do
        ...
        for _, calc in ipairs(faceCalcs) do
            ...
            if point:DistToSqr(pos) <= thresholdSquared then isFar = false end
            -- Skip faces aligned with the entity's "up" direction
            if calc.normal == up then continue end
            quad[#quad + 1] = point
            quad[#quad + 1] = offset
        end

        if not isFar then INSTANCE_BUILT_CONVEX_MESH[#INSTANCE_BUILT_CONVEX_MESH + 1] = quad end
    end
...
function ENT:Initialize()
    ...
    if CLIENT then self:SetRenderBoundsWS(Vector(-16000, -16000, -16000), Vector(16000, 16000, 16000)) end
end
