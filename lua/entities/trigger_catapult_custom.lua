-- ----------------------------------------------------------------------------
-- GP2 Framework
-- Trigger Catapult lua implementation
-- ----------------------------------------------------------------------------
ENT.Type = "brush"
local mats = {}
...
function ENT:PassesTriggerFilters(ent)
    ...
    if self:HasSpawnFlags(SF_TRIGGER_ALLOW_ALL) or (self:HasSpawnFlags(SF_TRIGGER_ALLOW_CLIENTS) and ent:IsPlayer()) or (self:HasSpawnFlags(SF_TRIGGER_ALLOW_NPCS) and ent:IsNPC()) or (self:HasSpawnFlags(SF_TRIGGER_ALLOW_PUSHABLES) and ent:GetClass() == "func_pushable") or (self:HasSpawnFlags(SF_TRIGGER_ALLOW_PHYSICS) and ent:GetMoveType() == MOVETYPE_VPHYSICS) then
        if ent:IsNPC() then if self:HasSpawnFlags(SF_TRIGGER_ONLY_PLAYER_ALLY_NPCS) and IsFriendEntityName(ent:GetClass()) == false then return false end end
        if self:HasSpawnFlags(SF_TRIGGER_ONLY_CLIENTS_IN_VEHICLES) and ent:IsPlayer() then if ent:InVehicle() == false then return false end end
        if self:HasSpawnFlags(SF_TRIGGER_ONLY_CLIENTS_OUT_OF_VEHICLES) and ent:IsPlayer() then if not ent:InVehicle() == true then return false end end
...
function ENT:StartTouch(other)
    ...
    if not self:PassesTriggerFilters(other) then return end
    ...
    if self.RefireDelay[refireIndex] > CurTime() then return end
    -- Don't touch things the player is holding
    if other:GetPhysicsObject() and other:IsPlayerHolding() then
        if not self.AbortedLaunchees[other] then self.AbortedLaunchees[other] = true end
        self.Thinking = true
    ...
    if IsValid(target) then
        ...
        if self:GetUseThresholdCheck() then
            ...
            if flDot < flLower or flDot > flUpper then shouldLaunch = false end
        end
...
function ENT:EndTouch(other)
    ...
    if not other:IsPlayer() and self.AbortedLaunchees[other] then self.AbortedLaunchees[other] = nil end
end
...
function ENT:CalculateLaunchVector(victim, target)
    ...
    if victim:IsPlayer() then vecTargetPos.z = vecTargetPos.z - 64 end
...
function ENT:CalculateLaunchVectorPreserve(vecInitialVelocity, victim, target, forcePlayer)
    ...
    if victim:IsPlayer() or forcePlayer then vecTargetPos.z = vecTargetPos.z - 64 end
    ...
    if vecInitialVelocity:Dot(vecTestVelocity1) > vecInitialVelocity:Dot(vecTestVelocity2) then return vecTestVelocity1 end
    return vecTestVelocity2
end
...
function ENT:LaunchByDirection(victim)
    ...
    if victim:IsPlayer() then
        ...
        if math.abs(vecPush.x) < 0.001 and math.abs(vecPush.y) < 0.001 then vecPush.z = self:GetPlayerSpeed() * 1.5 end
        -- Send us flying
        if victim:IsOnGround() then victim:SetGroundEntity(NULL) end
        hook.Add("Move", "GP2::test_setupmove_removespeed" .. victim:EntIndex(), function(ply, mv, cmd) if victim == ply then mv:SetVelocity(vecPush) end end)
...
function ENT:LaunchByTarget(victim, target)
    ...
    if victim:IsPlayer() then
        ...
        --victim:SetVelocity()
        hook.Add("Move", "GP2::test_setupmove_removespeed" .. victim:EntIndex(), function(ply, mv, cmd) if victim == ply then mv:SetVelocity(vecVelocity) end end)
        self:OnLaunchedVictim(victim)
    else
        if victim:GetMoveType() == MOVETYPE_VPHYSICS then
            -- Launch!
...
function ENT:Think()
    if self.Thinking then
        for other in pairs(self.AbortedLaunchees) do
            ...
            if bShouldRemove then self.AbortedLaunchees[other] = nil end
        end
    end
