...
function ENT:Think()
    ...
    for i = #self.TouchingEnts, 1, -1 do
        ...
        if not IsValid(ent) then
            ...
            if ent:IsPlayer() then GP2.GameMovement.PlayerExitedFromTractorBeam(ent, self) end
        else
...
function ENT:StartTouch(ent)
    if IsValid(self.TractorBeam) then
        ...
        elseif ent:IsPlayer() then
            ...
            if self.sndPlayerInBeam then self.sndPlayerInBeam:Stop() end
...
function ENT:EndTouch(ent)
    ...
    if ent:IsPlayer() then
        ...
        if self.sndPlayerInBeam then self.sndPlayerInBeam:FadeOut(5) end
    end
end
