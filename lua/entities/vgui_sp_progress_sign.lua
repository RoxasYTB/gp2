...
local function LoadLightboardIcons(sign)
    ...
    if not sp_lightboard_icons_content then return end
    ...
    if level.level_number then sign:SetLevelNumber(tonumber(level.level_number)) end
    if level.total_levels then sign:SetTotalLevels(tonumber(level.total_levels)) end
    if level.startup then sign:SetStartupSequence(level.startup) end
    if level.dirt then sign:SetDirt(tonumber(level.dirt)) end
...
function ENT:Think()
    if CLIENT then if VguiSPProgressSign and not VguiSPProgressSign.IsAddedSign(self) then VguiSPProgressSign.AddSign(self) end end
end
