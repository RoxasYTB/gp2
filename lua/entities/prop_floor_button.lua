...
if SERVER then
    ...
    function ENT:Initialize()
        if not self:GetModel() then self:SetModel(DEFAULT_BUTTON_MODEL) end
        ...
        timer.Simple(0.1, function() if IsValid(self.ButtonTrigger) then self.ButtonTrigger.CheckRadius = self.CheckRadius end end)
    ...
    ENT.__input2func = {
        ["pressin"] = function(self, activator, caller, data) self:Press() end,
        ["pressout"] = function(self, activator, caller, data) self:PressOut() end
    }
    ...
    function ENT:Press()
        ...
        if SERVER and GP2 and GP2.ButtonLogging then GP2.ButtonLogging.LogActivation("BOUTON AU SOL", self:GetName(), self:GetPos(), true) end
    end
    ...
    function ENT:PressOut()
        ...
        if SERVER and GP2 and GP2.ButtonLogging then GP2.ButtonLogging.LogActivation("BOUTON AU SOL", self:GetName(), self:GetPos(), false) end
    end
    ...
    function ENT:OnRemove()
        ...
        if IsValid(self.ButtonTrigger) then self.ButtonTrigger:Remove() end
    end
end
