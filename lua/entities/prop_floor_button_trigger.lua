...
if SERVER then
    ...
    timer.Simple(0, function() gp2_debug_buttons = GetConVar("gp2_debug_buttons") end)
...
local function DebugPrint(msg)
    if SERVER then
        ...
        if not gp2_debug_buttons then gp2_debug_buttons = GetConVar("gp2_debug_buttons") end
        if gp2_debug_buttons and gp2_debug_buttons:GetBool() then print(msg) end
    end
end
...
function ENT:SetButton(btn)
    ...
    if IsValid(btn) and btn.CheckRadius then self.CheckRadius = btn.CheckRadius end
end
...
function ENT:Think()
    ...
    for _, ply in ipairs(player.GetAll()) do
        if IsValid(ply) and ply:Alive() then
            ...
            if distance <= self.CheckRadius then table.insert(nearbyEnts, ply) end
        end
    end
        ...
        if entClass ~= "player" then
            for _, ent in ipairs(ents.FindByClass(entClass)) do
                if IsValid(ent) then
                    ...
                    if distance <= self.CheckRadius then table.insert(nearbyEnts, ent) end
                end
            end
        end
    end
    ...
    for i = #self.TouchingEnts, 1, -1 do
        ...
        if not IsValid(ent) then table.remove(self.TouchingEnts, i) end
    end

    if originalCount ~= #self.TouchingEnts then DebugPrint("[Floor Button] Cleaned invalid entities. Before: " .. originalCount .. ", After: " .. #self.TouchingEnts) end
    ...
    -- Activer/désactiver le bouton selon l'état requis
...
function ENT:StartTouch(ent)
    if not IsValid(ent) or not BUTTON_VALID_ENTS[ent:GetClass()] then return end
    if not IsValid(self.Button) or not isfunction(self.Button.IsButton) or not self.Button:IsButton() then return end
    -- Éviter les doublons
    for _, touchingEnt in ipairs(self.TouchingEnts) do
        if touchingEnt == ent then return end
    end
