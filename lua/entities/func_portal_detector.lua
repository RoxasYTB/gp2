...
function ENT:TriggerOutput(outputName, activator)
	...
	for _, v in ipairs(output) do
		self:FireOutput(v, activator);
	end;
end;
function ENT:StoreOutput(name, value)
	self.m_Outputs = self.m_Outputs or {};
	self.m_Outputs[name] = self.m_Outputs[name] or {};
	table.insert(self.m_Outputs[name], value);
end;
function ENT:FireOutput(output, activator)
	DoOutputAction(self, output, activator);
end;
function DoOutputAction(ent, output, activator)
	if ent.OutputAction then
		ent:OutputAction(output, activator);
	end;
end;
