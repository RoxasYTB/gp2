...
function ENT:KeyValue(k, v)
    ...
    if k == "spawnflags" and bit.band(tonumber(v), SF_INACTIVE) ~= 0 then self:SetEnabled(false) end
end

ENT.__input2func = {
    ["enable"] = function(self, activator, caller, data) self:SetEnabled(true) end,
    ["disable"] = function(self, activator, caller, data)
        if not self:GetEnabled() then return end
        self:SetEnabled(false)
    end,
    ["toggle"] = function(self, activator, caller, data) self:SetEnabled(not self:GetEnabled()) end,
}
