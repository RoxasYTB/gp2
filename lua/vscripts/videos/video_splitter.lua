ElevatorVideos = {
    ...
    ["sp_a1_intro3"] = {
        map = "sp_a1_intro3",
        arrival = "animalking",
        departure = "animalking",
        typeOverride = 11
    },
    ["sp_a1_intro4"] = {
        map = "sp_a1_intro4",
        arrival = "exercises_horiz",
        departure = "exercises_horiz",
        typeOverride = 10
    },
    ["sp_a1_intro5"] = {
        map = "sp_a1_intro5",
        arrival = "exercises_vert",
        departure = "exercises_vert",
        typeOverride = 9
    },
    ["sp_a1_intro6"] = {
        map = "sp_a1_intro6",
        arrival = "plc_blue_vert",
        departure = "plc_blue_vert",
        typeOverride = 9
    },
    ["sp_a1_intro7"] = {
        map = "sp_a1_intro7",
        arrival = "plc_blue_horiz",
        departure = "",
        typeOverride = 4
    },
    ["sp_a2_intro"] = {
        map = "sp_a2_intro",
        arrival = "",
        departure = "plc_blue_horiz",
        typeOverride = 1
    },
    ["sp_a2_laser_intro"] = {
        map = "sp_a2_laser_intro",
        arrival = "laser_portal",
    ...
    ["sp_a2_laser_over_goo"] = {
        ...
        arrival = "aperture_appear_vert",
        departure = "aperture_appear_vert",
        typeOverride = 9
    },
    ["sp_a2_catapult_intro"] = {
        map = "sp_a2_catapult_intro",
        arrival = "faithplate",
        departure = "faithplate",
        typeOverride = 6
    },
    ["sp_a2_trust_fling"] = {
    ...
    ["sp_a2_turret_blocker"] = {
        ...
        arrival = "turret_exploded_grey",
        departure = "turret_exploded_grey",
        typeOverride = 6
    },
    ["sp_a2_laser_vs_turret"] = {
        map = "sp_a2_laser_vs_turret",
        arrival = "turret_colours_type",
        departure = "turret_colours_type",
        typeOverride = 6
    },
    ["sp_a2_pull_the_rug"] = {
    ...
    ["sp_a2_column_blocker"] = {
        ...
        arrival = "turret_dropin",
        departure = "turret_dropin",
        typeOverride = 6
    },
...
function Precache()
    ...
    if level then
        local movieName
        if level.additional and level.additional ~= "" then movieName = "media/" .. level.additional end
        if level.arrival and level.arrival ~= "" then movieName = "media/" .. (OVERRIDE_VIDEOS and "entry_emergency" or level.arrival) end
        if level.departure and level.departure ~= "" then movieName = "media/" .. (OVERRIDE_VIDEOS and "exit_emergency" or level.departure) end
...
function StartDestructedArrivalVideo(width, height)
    ...
    if game.GetMap() == FIRST_CLEAN_MAP then playDestructed = false end
    local level = ElevatorVideos[game.GetMap()]
...
function StartVideo(videoType, width, height)
    ...
    local level = ElevatorVideos[game.GetMap()]
    if level and level.typeOverride then videoScaleType = level.typeOverride end
    if level and level.destructChance then randomDestructChance = level.destructChance end
    for i = 0, width do
        for j = 0, height do
            local panelNum = 1 + width * j + i
            local signName
            ...
            elseif videoScaleType == 1 then
                -- stretch
                uMin = 1.0 - (1.0 - uMin) * (1.0 - uMin) * (1.0 - uMin)
                uMax = 1.0 - (1.0 - uMax) * (1.0 - uMax) * (1.0 - uMax)
            elseif videoScaleType == 2 then
                -- mirror
                uMin = 1.0 - (1.0 - uMin) * (1.0 - uMin) * (1.0 - uMin)
                uMax = 1.0 - (1.0 - uMax) * (1.0 - uMax) * (1.0 - uMax)
            elseif videoScaleType == 3 then
                -- ouroboros
                uMin = ((i % 12) + 0.0001) / 12
                uMax = ((i % 12) + 1.0001) / 12
                if i % 2 == 1 then
            ...
            elseif videoScaleType == 4 then
                -- upside down
                vMin = 0.99999
                vMax = 0.00001
                uMin = ((i % 3) + 0.0001) / 3
                uMax = ((i % 3) + 1.0001) / 3
            elseif videoScaleType == 5 then
                -- tiled
                vMin = 0.00001
                vMax = 0.99999
                uMin = ((i % 3) + 0.0001) / 3
                uMax = ((i % 3) + 1.0001) / 3
            elseif videoScaleType == 6 then
                -- tiled really big
                uMin = ((i % 8) + 0.0001) / 8
                uMax = ((i % 8) + 1.0001) / 8
            elseif videoScaleType == 7 then
                -- tiled big
                uMin = ((i % 12) + 0.0001) / 12
                uMax = ((i % 12) + 1.0001) / 12
            elseif videoScaleType == 8 then
                -- tiled single
                uMin = 0.0001
                uMax = 0.9999
                vMin = 0.0001
                vMax = 0.9999
            elseif videoScaleType == 9 then
                -- tilled double
                uMin = ((i % 2) + 0.0001) / 2
                uMax = ((i % 2) + 1.0001) / 2
            elseif videoScaleType == 10 then
                -- two by two
                vMin = 0.00001
                vMax = 0.99999
                uMin = ((i % 2) + 0.0001) / 2
                uMax = ((i % 2) + 1.0001) / 2
            elseif videoScaleType == 11 then
                -- tiled off
                vMin = 0.00001
                vMax = 0.99999
                uMin = (((i + 1) % 3) + 0.0001) / 3
                uMax = (((i + 1) % 3) + 1.0001) / 3
            elseif videoScaleType == 12 then
                -- tiled 2x4
                uMin = ((i % 6) + 0.0001) / 6
                uMax = ((i % 6) + 1.0001) / 6
            elseif videoScaleType == 13 then
                -- tiled double
                if (i % 4) < 2 then
                    uMin = ((i % 2) + 0.0001) / 2
                    uMax = ((i % 2) + 1.0001) / 2
