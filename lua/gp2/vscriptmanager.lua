...
local function relativeInclude(baseDir, fileName)
    ...
    if not file.Exists(actualPath, "LUA") then error("Included script not found: " .. actualPath) end
...
GP2.VScriptMgr = {
    Initialize = function() GP2.Print("Initializing the VScript system") end,
    --
    -- Run script file
    -- 
    InitializeScriptForEntity = function(ent, v)
        ...
        timer.Simple(0, function() GP2.VScriptMgr.CallScriptFunction(ent, "Precache", true, NULL) end)
    end,
    --
    -- Add think function to queue
    -- 
    InitializeScriptThinkFuncForEntity = function(ent, v) thinkqueues[ent] = v end,
    ClearScriptScope = function(ent) vscriptinstance[ent] = {} end,
    --
    ...
    RunScriptFile = function(ent, fl)
        ...
        if not success then GP2.VScriptMgr.Error(runtimeErr) end
    end,
    --
    ...
    RunScriptCode = function(ent, code)
        ...
        scope.self = ent
        -- Relative include
        scope.include = function(fileName) GP2.VScriptMgr.Error("Include function is not supported in RunScriptCode.") end
        ...
        setfenv(chunk, scope)
        local success, runtimeErr = pcall(chunk)
        if not success and runtimeErr then GP2.VScriptMgr.Error(runtimeErr:gsub("CompileString%:%d+: ", "RunScriptCode: ( '" .. (ent:GetName() ~= "" and ent:GetName() or ent:GetClass()) .. "' )")) end
    ...
    CallHookFunction = function(hookname, failesilent, ...)
        for ent in pairs(entswithvscripts) do
            if IsValid(ent) or ent:IsWorld() then GP2.VScriptMgr.CallScriptFunctionWithArgs(ent, hookname, failesilent, ...) end
        end
    end,
    Think = function()
...
GP2.InputsHandler.AddCallback("runscriptcode", function(ent, activator, caller, value) GP2.VScriptMgr.RunScriptCode(ent, value) end)
GP2.InputsHandler.AddCallback("runscriptfile", function(ent, activator, caller, value) GP2.VScriptMgr.RunScriptFile(ent, value) end)
GP2.InputsHandler.AddCallback("callscriptfunction", function(ent, activator, caller, value) GP2.VScriptMgr.CallScriptFunction(ent, value, false, caller) end)
