...
timer.Create("portals_ent_update", ENTS_UPDATE_INTERVAL, 0, function()
    ...
    for _, ent in ipairs(ents.GetAll()) do
        if IsValid(ent) and ent:GetPhysicsObject() and IsValid(ent:GetPhysicsObject()) then
            ...
            if class == "prop_physics" or class == "prop_weighted_cube" or class == "simple_physics_prop" or class == "prop_ragdoll" then table.insert(allEnts, ent) end
        end
    end
...
local function teleportEntity(ent, hitPortal, linkedPortal)
    ...
    local transformedPos, transformedAng = PortalManager.TransformPortal(hitPortal, linkedPortal, ent:GetPos(), ent:GetAngles())
    local _, transformedVelAngle = PortalManager.TransformPortal(hitPortal, linkedPortal, nil, ent:GetVelocity():Angle())
    ...
    for _, prop in ipairs(allEnts) do
        ...
        if tr.Hit and IsValid(tr.Entity) and tr.Entity:GetClass() == "prop_portal" then
            ...
            if IsValid(linkedPortal) and hitPortal:GetActivated() then
                -- Check direction
                if propVel:Dot(hitPortal:GetUp()) < -0.5 then teleportEntity(prop, hitPortal, linkedPortal) end
            end
        end
    end
end)
