...
if SERVER then
    ...
    local function precacheClosecaptionFiles()
        ...
        for i, name in ipairs(fls) do
            if name:StartsWith('subtitles_') and name:EndsWith('.txt') then availableLanguages[name:Replace('subtitles_', ''):Replace(".txt")] = true end
        end
    end
    ...
    function SoundManager.Think()
        ...
        for token, removalTime in pairs(recentCaptions) do
            if CurTime() > removalTime then recentCaptions[token] = nil end
        end
    end
