...
function EnvPortalLaser.RefreshExistingLasers()
    for _, ent in ents.Iterator() do
        if IsValid(ent) and ent:GetClass() == "env_portal_laser" then EnvPortalLaser.AddToRenderList(ent) end
    end
end
...
local function portal_enhanced_object_detection(laser, tr, linkedPortal)
    ...
    for e = 1, #inTrace do
        ...
        local intersect = util.IntersectRayWithOBB(tr.StartPos, tr.Normal * MAX_RAY_LENGTH, tracedEntity:GetPos(), tracedEntity:GetAngles(), mins, maxs)
        if intersect then
    ...
    table.sort(candidates, function(a, b) return a.DistanceSqr < b.DistanceSqr end)
    ...
    for _, candidate in ipairs(candidates) do
        ...
        if not preEndTrace.Hit or not IsValid(preEndTrace.Entity) or not filter[preEndTrace.Entity:GetClass()] then continue end
        rayHit = {
            HitPos = intersect,
            Entity = tracedEntity,
...
function EnvPortalLaser.Render()
    for i = #laserRenderList, 1, -1 do
        ...
        RecursionLaserThroughPortals(laser, NULL, {
            ...
            filter = {laser, "projected_wall_entity", "player", "point_laser_target", "prop_laser_catcher", "prop_laser_relay", laser:GetParent()},
