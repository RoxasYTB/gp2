        ...
        for i = 1, 3 do
            ...
            if attach then
                if beam:GetEnabled() then
                    ...
                    table.insert(beam.AttachmentTrails[i], {
                        Pos = attach.Pos,
                        LifeTime = 2,
                        MaxLifeTime = 2
                    })
                ...
                for b = #beam.AttachmentTrails[i], 1, -1 do
                    if beam.AttachmentTrails[i][b].LifeTime <= 0 then table.remove(beam.AttachmentTrails[i], b) end
                end
            end
        end
    end
...
hook.Add("PreDrawTranslucentRenderables", "GP2::PreDrawTranslucentRenderables", function(depth, sky, skybox3d)
    ...
    PropPortal.Render()
    if PORTAL_USE_NEW_ENVIRONMENT_SYSTEM and PortalRendering.Rendering then return true end
end)

hook.Add("PreDrawOpaqueRenderables", "GP2::PreDrawOpaqueRenderables", function(depth, sky, skybox3d) if PORTAL_USE_NEW_ENVIRONMENT_SYSTEM and PortalRendering.Rendering then return true end end)
hook.Add("PostDrawTranslucentRenderables", "GP2::PostDrawTranslucentRenderables", function(depth, sky, skybox3d)
    ...
    if envPortalLaserIncluded and EnvPortalLaser and type(EnvPortalLaser.Render) == "function" then
        ...
        if not success then print("[GP2-SDK] Error in EnvPortalLaser.Render: " .. tostring(err)) end
    end
...
hook.Add("PostDrawOpaqueRenderables", "GP2::PostDrawOpaqueRenderables", function(depth, sky, skybox3d) end)
