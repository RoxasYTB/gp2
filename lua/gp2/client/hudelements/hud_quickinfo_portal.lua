...
function PANEL:Paint(w, h)
    ...
    if not (IsValid(weapon) and VALID_CLASSES[weapon:GetClass()]) then return end
    surface_SetMaterial(crosshairMaterial)
    ...
    if not self.gp2_portal_color1 then self.gp2_portal_color1 = GetConVar("gp2_portal_color1") end
    if not self.gp2_portal_color2 then self.gp2_portal_color2 = GetConVar("gp2_portal_color2") end
...
function PANEL:ShouldDraw()
    if not self.BaseClass.ShouldDraw() then return false end
    return true
end
