...
timer.Create("seamless_portal_distance_fix", PORTAL_SORT_INTERVAL, 0, function()
    ...
    if #newPortals ~= lastPortalCount or CurTime() - lastPortalSort > PORTAL_SORT_INTERVAL then
        portals = newPortals
        table.sort(portals, function(a, b) return a:GetPos():DistToSqr(EyePos()) < b:GetPos():DistToSqr(EyePos()) end)
...
hook.Add("RenderScene", "seamless_portal_draw", function(eyePos, eyeAngles, fov)
    if not PortalManager or PortalManager.PortalIndex < 1 then return end
    if gp2_portal_drawdistance:GetFloat() < 10 then -- désactive le rendu si drawdistance trop faible
        return
    end
    ...
    for _, portal in ipairs(portals) do
        ...
        if rtTimesRendered < PortalRendering.MaxRTs and PortalManager.ShouldRender(portal, eyePos, eyeAngles, gp2_portal_drawdistance:GetFloat()) then
            local linkedPartner = portal:GetLinkedPartner()
            local transformedPos, transformedAng = PortalManager.TransformPortal(portal, linkedPartner, eyePos, eyeAngles)
...
hook.Add("ShouldDrawLocalPlayer", "seamless_portal_drawplayer", function() if PortalRendering.Rendering and not PortalRendering.DrawPlayerInView then return true end end)
-- (REWRITE THIS!)
...
hook.Add("PostDrawTranslucentRenderables", "seamless_portal_skybox", function()
    if not PortalRendering.Rendering or util.IsSkyboxVisibleFromPoint(renderViewTable.origin) then return end
