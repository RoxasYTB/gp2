...
function EOT_Integration.SetupGelIntegration()
    ...
    hook.Add("PlayerSprayPaint", "EOT_GelTracking", function(ply, tr)
        ...
        local ent = tr.Entity
        if not ent.EOT_GelProperties then ent.EOT_GelProperties = {} end
        -- Track gel type and application time
        local gel_type = "unknown"
        local weapon = ply:GetActiveWeapon()
        if IsValid(weapon) then if weapon:GetClass() == "weapon_paintgun" then gel_type = weapon:GetPaintType() or "unknown" end end
...
function EOT_Integration.SetupFizzlerIntegration()
    ...
    hook.Add("EOT_ShouldTransmit", "EOT_FizzlerBlocker", function(ent, portal)
        ...
        if tr.Hit and IsValid(tr.Entity) then
            ...
            if hit_class == "trigger_portal_cleanser" or hit_class == "func_portal_cleanser" then
                -- Block transmission
                return false
            end
        end
...
-- Auto-initialize
if SERVER then hook.Add("Initialize", "EOT_IntegrationInit", function() EOT_Integration.Initialize() end) end
