...
local gp2_portal_movement = CreateConVar("gp2_portal_movement", "1", FCVAR_REPLICATED + FCVAR_NOTIFY, "Toggle custom Source movement reimplementation for portal environments")
...
function PortalMovement.LookForPortalEnvironment(ply, mv)
    ...
    if IsValid(tr.Entity) then debugoverlay.Box(plyPos, mins, maxs, 0.05, DEBUG_COLOR_NOT_IN_PORTAL) end
...
function PortalMovement.Move(ply, mv)
    if not (gp2_portal_movement:GetBool() and ply.PortalEnvironment) then return false end
    local plyPos = ply:GetPos()
    local mins, maxs = ply:GetHull()
    ...
    if trace.Hit then
        if not IsValid(ply:SetGroundEntity()) or ply:GetGroundEntity() ~= trace.Entity then ply:SetGroundEntity(trace.Entity) end
    else
    ...
    if not onGround then vel.z = vel.z - (sv_gravity:GetFloat() * FrameTime()) end
    ...
    if moveTrace.Hit and not moveTrace.HitWorld then
        ...
        if dot < 0 then vel = vel - moveTrace.HitNormal * dot end
        ply:SetGroundEntity(moveTrace.Entity)
    else
        ply:SetGroundEntity(NULL)
    end
...
hook.Add("CalcView", "TEMP_Z_NEAR_FIX", function(ply, pos, angles, fov, znear, zfar)
    if not ply:GetNWBool("GP2:InPortalEnvironment") then
        return {
            znear = znear
        }
    end
