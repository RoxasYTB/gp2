...
hook.Add("OnEntityCreated", "seamless_portals", function(ent) if ent:GetClass() == "prop_portal" then table.insert(portals, ent) end end)
-- remove from the portals list when a portal is removed
hook.Add("EntityRemoved", "seamless_portals", function(ent) if ent:GetClass() == "prop_portal" then table.RemoveByValue(portals, ent) end end)
...
hook.Add("SetupPlayerVisibility", "seamless_portals", function(ply, viewEntity)
    ...
    for _, portal in ipairs(portals) do
        if portal:IsValid() then
            ...
            if IsValid(linkedPartner) and ply:TestPVS(portal) and PortalManager.ShouldRender(portal, eyePos, eyeAngle, distance) then AddOriginToPVS(linkedPartner:GetPos()) end
        end
    end
end)
