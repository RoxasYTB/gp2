...
local function IsPointInShape(point, vertices, normal, tolerance)
    ...
    if math.abs(distance) > tolerance then return false end
    ...
    for i = 1, #vertices do
        ...
        if start2D.y <= point2D.y then
            if finish2D.y > point2D.y and cross2D(finish2D - start2D, point2D - start2D) > 0 then windingNumber = windingNumber + 1 end
        else
            if finish2D.y <= point2D.y and cross2D(finish2D - start2D, point2D - start2D) < 0 then windingNumber = windingNumber - 1 end
        end
    end
...
function PaintManager.GetSurfaceByTrace(start, direction)
    local tr = PortalManager.TraceLinePortal({
        start = start,
        ...
        for s = 1, #surfinfo do
            ...
            if gp2_blobulator_debug:GetBool() then
                ...
                for i = 1, #vertices do
                    ...
                    local nextIndex = i + 1
                    if nextIndex > #vertices then nextIndex = 1 end
...
local cache = {} -- Cache to store surfaces data
local cacheLifetime = 5 -- Time in seconds for cache validity
function PaintManager.GetSurfacesInRadius(center, radius)
    local cacheKey = tostring(center) .. ":" .. tostring(radius)
    local currentTime = CurTime()
    -- Check the cache
    if cache[cacheKey] and currentTime - cache[cacheKey].time < cacheLifetime then return cache[cacheKey].surfaces end
    local surfacesInRadius = {}
    local trace = {
        start = center,
        endpos = center,
    ...
    local entities = ents.FindInBox(trace.start + trace.mins, trace.start + trace.maxs)
    for _, entity in ipairs(entities) do
        if not entity.GetBrushSurfaces then continue end
        local surfinfo = entity:GetBrushSurfaces()
        for s = 1, #surfinfo do
            local vertices = surfinfo[s]:GetVertices()
            local hitPos = center -- Test point for radius check
            -- Check if the center point is within the surface
            if IsPointInShape(hitPos, vertices, surfinfo[s]:GetNormal(), radius) then table.insert(surfacesInRadius, surfinfo[s]) end
        end
    end
    ...
    cache[cacheKey] = {
        time = currentTime,
        surfaces = surfacesInRadius
    }
    return surfacesInRadius
end
