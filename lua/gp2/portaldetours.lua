...
local function effect(name, b, c, d)
    if PortalManager.PortalIndex > 0 and name and tabEffectClass[name] then return end
...
hook.Add("EntityEmitSound", "GP2::PortalDetourSound", function(t)
    ...
    for k, v in ipairs(ents.FindByClass("prop_portal")) do
        ...
        if not v.ExitPortal or not exitportal or not exitportal:IsValid() or not exitportal.GetExitSize then continue end
        if not t.Pos or not t.Entity or t.Entity == NULL then continue end
