...
if SERVER then
    ...
    function GP2.Utils.ToViewModelPosition(vOrigin)
        ...
        if nViewX == 0 or nWorldX == 0 then return vEyePos + vForward * vForward:Dot(vOffset) end
        local nFactor = nViewX / nWorldX
        return vEyePos + aEyesRot:Right() * (aEyesRot:Right():Dot(vOffset) * nFactor) + aEyesRot:Up() * (aEyesRot:Up():Dot(vOffset) * nFactor) + vForward * vForward:Dot(vOffset)
    end
end
