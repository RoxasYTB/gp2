...
function PortalManager.GetLinkageGroup(linkageGroup)
    if not PortalManager.LinkageGroups[linkageGroup] then PortalManager.LinkageGroups[linkageGroup] = {NULL, NULL} end
...
function PortalManager.SetPortal(linkageGroup, entity)
    if not IsValid(entity) or entity:GetClass() ~= "prop_portal" then return end
    -- Get type of portal
    ...
    -- fizzle it
    if IsValid(portal) and portal ~= entity then if SERVER then portal:Fizzle() end end
    ...
    -- to prevent next
    if entity:GetActivated() then PortalManager.GetLinkageGroup(linkageGroup)[portalType] = entity end
end

function PortalManager.TransformPortal(a, b, pos, ang, offsetByThick)
    if not IsValid(a) or not IsValid(b) then return Vector(), Angle() end
    local editedPos = Vector()
    local editedAng = Angle()
    if pos then
        editedPos = a:WorldToLocal(pos)
        if offsetByThick then editedPos = editedPos + a:GetAngles():Forward() * 7 end
