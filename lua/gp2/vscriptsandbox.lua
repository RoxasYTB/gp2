...
sandbox.CreateSceneEntity = function(path)
    ...
    SceneEntityMT.__index = function(tbl, key)
        if SceneEntityMT[key] then return SceneEntityMT[key] end
...
sandbox.ScriptShowHudMessageAll = function(message, delay)
    delay = delay or 0
    timer.Simple(delay, function() PrintMessage(HUD_PRINTCENTER, message) end)
end

sandbox.TurnOnPotatos = function() SetGlobalBool("GP2::PotatosEnabled", true) end
sandbox.TurnOffPotatos = function() SetGlobalBool("GP2::PotatosEnabled", false) end
