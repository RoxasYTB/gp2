...
hook.Add("AcceptInput", "GP2::AcceptInput", function(ent, name, activator, caller, value)
    ...
    if callback and isfunction(callback) then callback(ent, activator, caller, value) end
    if name:find("^O_") then ent:TriggerConnectedOutput(name:gsub("^O_", "")) end
            ...
            if vcdContent then
                for actorName in vcdContent:gmatch('actor%s+"(.-)"') do
                    for _, entity in ipairs(ents.GetAll()) do
                        if entity:GetName() == actorName then entity:SetCurrentScene(NULL) end
                    end
                end
            end
        end
    ...
    if name == "O_OnStart" then
        ...
        if vcd then
            ...
            if vcdContent then
                for actorName in vcdContent:gmatch('actor%s+"(.-)"') do
                    for _, entity in ipairs(ents.GetAll()) do
                        if entity:GetName() == actorName then entity:SetCurrentScene(ent) end
                    end
                end
            end
        end
    ...
    if ent:GetClass() == "env_tonemap_controller" and name == "setbloomscale" then
        ...
        if not ent.DontCallBloomScaleAgain then
            ...
            timer.Simple(0, function() ent.DontCallBloomScaleAgain = nil end)
            return true
        end
    end
end)
