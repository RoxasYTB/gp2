...
if not EOT_Config.Initialize then
    function EOT_Config.Initialize()
        ...
        if SERVER then
            ...
            local requiredCvars = {"gp2_object_transmission_enabled", "gp2_object_transmission_max_per_frame", "gp2_object_transmission_velocity_threshold"}
            for _, cvarName in ipairs(requiredCvars) do
                local cvar = GetConVar(cvarName)
                if not cvar then print("[Enhanced Object Transmission] Warning: ConVar " .. cvarName .. " not found") end
            end
        end
    end
end
...
if SERVER then
    ...
    CreateConVar("gp2_object_transmission_enabled", "1", FCVAR_ARCHIVE + FCVAR_NOTIFY, "Enable enhanced object transmission through portals")
    CreateConVar("gp2_object_transmission_max_per_frame", "8", FCVAR_ARCHIVE, "Maximum objects to process for transmission per frame")
    CreateConVar("gp2_object_transmission_velocity_threshold", "25", FCVAR_ARCHIVE, "Minimum velocity required for object transmission consideration")
    CreateConVar("gp2_object_transmission_size_multiplier", "2.0", FCVAR_ARCHIVE, "Size multiplier for portal transmission (higher = larger objects can pass)")
    CreateConVar("gp2_object_transmission_distance_threshold", "128", FCVAR_ARCHIVE, "Maximum distance from portal plane for transmission consideration")
    CreateConVar("gp2_object_transmission_update_interval", "0.02", FCVAR_ARCHIVE, "Update interval for object transmission processing (seconds)")
    -- Advanced physics settings
    CreateConVar("gp2_object_transmission_preserve_constraints", "1", FCVAR_ARCHIVE, "Preserve constraint relationships during object transmission")
    CreateConVar("gp2_object_transmission_velocity_boost", "1.0", FCVAR_ARCHIVE, "Velocity multiplier applied to transmitted objects")
    CreateConVar("gp2_object_transmission_gravity_compensation", "1", FCVAR_ARCHIVE, "Apply gravity compensation to transmitted objects")
    -- Entity type filters
    CreateConVar("gp2_object_transmission_allow_physics", "1", FCVAR_ARCHIVE, "Allow prop_physics entities to be transmitted")
    CreateConVar("gp2_object_transmission_allow_cubes", "1", FCVAR_ARCHIVE, "Allow prop_weighted_cube entities to be transmitted")
    CreateConVar("gp2_object_transmission_allow_ragdolls", "1", FCVAR_ARCHIVE, "Allow prop_ragdoll entities to be transmitted")
    CreateConVar("gp2_object_transmission_allow_turrets", "1", FCVAR_ARCHIVE, "Allow npc_portal_turret_floor entities to be transmitted")
    CreateConVar("gp2_object_transmission_allow_energy_balls", "1", FCVAR_ARCHIVE, "Allow prop_energy_ball entities to be transmitted")
    -- Debug and performance settings
    CreateConVar("gp2_object_transmission_debug", "0", FCVAR_CHEAT, "Enable debug output for object transmission")
    CreateConVar("gp2_object_transmission_debug_visual", "0", FCVAR_CHEAT, "Enable visual debugging for object transmission")
    CreateConVar("gp2_object_transmission_performance_monitoring", "0", FCVAR_ARCHIVE, "Enable performance monitoring for transmission system")
    -- Portal-specific settings inspired by Portal source code
    CreateConVar("gp2_portal_trace_vs_world", "1", FCVAR_ARCHIVE, "Use traces against portal environment world geometry")
    CreateConVar("gp2_portal_trace_vs_objects", "1", FCVAR_ARCHIVE, "Use traces against portal environment objects")
    CreateConVar("gp2_use_transformed_collideables", "1", FCVAR_ARCHIVE, "Use transformed collideable system for portal object detection")
end

-- Client-side visual and audio settings
if CLIENT then
    CreateClientConVar("gp2_object_transmission_show_effects", "1", true, false, "Show visual effects during object transmission")
    CreateClientConVar("gp2_object_transmission_show_debug_overlay", "0", true, false, "Show debug overlay for object transmission areas")
    CreateClientConVar("gp2_object_transmission_sound_enabled", "1", true, false, "Play sound effects during object transmission")
    CreateClientConVar("gp2_object_transmission_prediction", "1", true, false, "Enable client-side transmission prediction")
end
...
function EOT_Config.GetConfig(setting)
    ...
    if cvar then return cvar:GetBool(), cvar:GetFloat(), cvar:GetString() end
    return false, 0, ""
end

function EOT_Config.IsEnabled()
    if SERVER then return GetConVar("gp2_object_transmission_enabled"):GetBool() end
...
local function validateConfiguration()
    ...
    if settings.MaxObjectsPerFrame < 1 or settings.MaxObjectsPerFrame > 50 then SafePrint("gp2_object_transmission_max_per_frame should be between 1 and 50") end
    if settings.VelocityThreshold < 0 or settings.VelocityThreshold > 1000 then SafePrint("gp2_object_transmission_velocity_threshold should be between 0 and 1000") end
    if settings.SizeMultiplier < 0.1 or settings.SizeMultiplier > 10.0 then SafePrint("gp2_object_transmission_size_multiplier should be between 0.1 and 10.0") end
    if settings.UpdateInterval < 0.001 or settings.UpdateInterval > 1.0 then SafePrint("gp2_object_transmission_update_interval should be between 0.001 and 1.0") end
end
...
if SERVER then
    cvars.AddChangeCallback("gp2_object_transmission_enabled", function(cvar, oldVal, newVal) SafePrint("Object transmission system " .. (tobool(newVal) and "enabled" or "disabled")) end)
    cvars.AddChangeCallback("gp2_object_transmission_max_per_frame", function(cvar, oldVal, newVal)
        local newValue = tonumber(newVal)
        if newValue and (newValue < 1 or newValue > 50) then SafePrint("WARNING: Max objects per frame value out of recommended range (1-50)") end
    end)
end
