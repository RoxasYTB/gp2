...
if SERVER then
    ...
    net.Receive("PORTALS_FREEZE", function()
        ...
        --singleplayer fixes (cuz stupid move hook isnt clientside in singleplayer)
        freezePly = false
    end)
end
...
local function editPlayerCollision(mv, ply, t)
    ...
    if not ply.PORTAL_STUCK_OFFSET then
        ...
        if ply.PORTAL_STUCK_OFFSET ~= 0 then
            local tr = PortalManager.TraceLine({
                start = ply:EyePos(),
                endpos = ply:EyePos() - Vector(0, 0, 64),
                filter = ply
            })
    ...
    if not ply.PORTAL_STUCK_OFFSET and tr.Hit and tr.Entity:GetClass() == "prop_portal" and tr.Entity.GetLinkedPartner and IsValid(tr.Entity:GetLinkedPartner()) then
        ...
        if dotUp > 0.5 then -- the portal is on the ground
            ...
            local tr = util.TraceHull(traceTable)
            if not tr.Hit or tr.Entity:GetClass() ~= "prop_portal" then
...
hook.Add("Move", "seamless_portal_teleport", function(ply, mv)
    if not PortalManager or PortalManager.PortalIndex < 1 then
    ...
    if hitPortal:GetClass() == "prop_portal" and hitPortal.GetLinkedPartner and IsValid(hitPortal:GetLinkedPartner()) and plyVel:Dot(hitPortal:GetUp()) < 0 and hitPortal:GetActivated() then
        ...
        timer.Simple(0.03, function() ply.PORTAL_TELEPORTING = false end)
        return true
    end
end)
