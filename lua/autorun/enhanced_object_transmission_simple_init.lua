...
if SERVER then
    ...
    if not _G.EOT_Config.Initialize then
        _G.EOT_Config.Initialize = function() print("[EOT] Config initialized") end
    end
    if not _G.EOT_Test.QuickTest then
        _G.EOT_Test.QuickTest = function(ply) print("[EOT] QuickTest executed") end
    end
    if not _G.EOT_Integration.EnableIntegration then
        _G.EOT_Integration.EnableIntegration = function(name) print("[EOT] Enabled: " .. tostring(name)) end
    end
    if not _G.EOT_Integration.DisableIntegration then
        _G.EOT_Integration.DisableIntegration = function(name) print("[EOT] Disabled: " .. tostring(name)) end
    end
    if not _G.EOT_Validator.RunValidation then
        _G.EOT_Validator.RunValidation = function(ply) print("[EOT] Validation executed") end
    end
    if not _G.EOT_FinalStatus.RunDiagnosis then
        _G.EOT_FinalStatus.RunDiagnosis = function() print("[EOT] Final status diagnosis executed") end
    end
    if not _G.EOT_FinalIntegrationTest.RunAllTests then
        _G.EOT_FinalIntegrationTest.RunAllTests = function() print("[EOT] Final integration test executed") end
    end
...
concommand.Add("eot_integration_enable", function(ply, cmd, args)
    ...
    if _G.EOT_Integration and _G.EOT_Integration.EnableIntegration then
        _G.EOT_Integration.EnableIntegration(integration)
    end
...
concommand.Add("eot_integration_disable", function(ply, cmd, args)
    ...
    local integration = args[1] or "unknown"
    if _G.EOT_Integration and _G.EOT_Integration.DisableIntegration then
        _G.EOT_Integration.DisableIntegration(integration)
    end
