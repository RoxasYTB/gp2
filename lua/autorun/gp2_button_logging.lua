...
-- ----------------------------------------------------------------------------
if CLIENT then -- Côté serveur uniquement
    return
end
...
function GP2.ButtonLogging.LogActivation(buttonType, buttonName, pos, activated)
    ...
    if gp2_log_buttons_detailed:GetBool() then
        print(string.format("[GP2 BUTTON LOG] %s %s - Nom: %s - Position: %.0f %.0f %.0f", buttonType, action, name, pos.x, pos.y, pos.z))
    else
        print(string.format("[GP2 BUTTON LOG] %s %s - Nom: %s", buttonType, action, name))
    end
end
