...
if SERVER then
    ...
    local function debugPrint(message, ...)
        ...
        if cvars.debug and cvars.debug:GetBool() then GP2.Print(message, ...) end
    end
    ...
    hook.Add("PlayerSpawn", "GP2::UnifiedSpawn::CaptureFirstSpawn", function(ply, transition)
        ...
        if not cvars.enabled or not cvars.enabled:GetBool() then return end
        ...
        elseif soloSpawnPos then
            ...
            timer.Simple(delay, function()
                if IsValid(ply) then
                    ...
                    if soloSpawnAngles then offset:Rotate(soloSpawnAngles) end
    ...
    concommand.Add("gp2_reset_spawn_position", function(ply, cmd, args)
        if not IsValid(ply) or ply:IsAdmin() or ply:IsSuperAdmin() then
            ...
            if IsValid(ply) then ply:PrintMessage(HUD_PRINTCONSOLE, "Position de spawn solo réinitialisée") end
        else
