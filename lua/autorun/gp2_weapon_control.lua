...
if SERVER then
    ...
    CreateConVar("gp2_remove_default_weapons", "1", FCVAR_ARCHIVE + FCVAR_NOTIFY, "Retire les armes par défaut de Garry's Mod (0 = non, 1 = oui)")
    CreateConVar("gp2_portal_gun_only", "1", FCVAR_ARCHIVE + FCVAR_NOTIFY, "Mode Portal Gun uniquement - retire toutes les autres armes (0 = non, 1 = oui)")
                ...
                timer.Simple(0.1, function()
                    if IsValid(ply) then
                        ...
                        for _, weapon in ipairs(ply:GetWeapons()) do
                            if weapon:GetClass() == "weapon_portalgun" then weapon:UpdatePortalGun() end
                        end
                        ...
                        timer.Simple(0.2, function()
                            if IsValid(ply) then
                                ...
                                if IsValid(portalgun) then ply:SelectWeapon("weapon_portalgun") end
                            end
                        end)
                    end
                end)
        ...
        if GetConVar("gp2_portal_gun_only"):GetBool() then
            timer.Simple(0.3, function()
                if IsValid(ply) then
                    ...
                    for _, weapon in ipairs(ply:GetWeapons()) do
                        if not allowedWeapons[weapon:GetClass()] then ply:StripWeapon(weapon:GetClass()) end
                    end
                end
            end)
        end
...
if CLIENT then
    ...
    local function SuppressWeaponHelpNotifications()
        ...
        hook.Add("HUDShouldDraw", "GP2::SuppressWeaponHelp", function(name)
            ...
            if suppressedElements[name] then return false end
        end)
        ...
        hook.Add("Think", "GP2::SuppressWeaponHelpBubbles", function()
            ...
            if ply:GetInfoNum("gmod_showhints", 1) == 1 then
                ...
                local success = pcall(function() RunConsoleCommand("gmod_showhints", "0") end)
    ...
    hook.Add("InitPostEntity", "GP2::InitWeaponControl", function() timer.Simple(1, SuppressWeaponHelpNotifications) end)
    ...
    concommand.Add("gp2_show_weapon_help", function()
        local success = pcall(function() RunConsoleCommand("gmod_showhints", "1") end)
        if success then
